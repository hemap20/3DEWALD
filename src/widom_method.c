...
/* energy interactions of particle i due to other atoms in the system 
rx, ry, rz: Arrays containing the x, y, and z coordinates of the particles
rc2: Square of the cutoff radius for the Lennard-Jones potential
tailcorr: Flag indicating whether to apply the tail correction
ecor: Tail correction value for energy
shift: Flag indicating whether to shift the potential to zero at the cutoff distance.
ecut: Energy shift value at the cutoff distance.
vir: Pointer to the virial term (related to pressure calculations in NVT ensemble).
*/
//energy within rc
  ...
  *vir=0.0;
  for (j=i0;j<N;j++) {
    if (i!=j) {
      ...
      /*the distance between two particles is measured as the 
...
//tailcorr: to approximate the energy calculations beyond cutoff radius
//shift: ecut is subtracted from the calculated energy to make the LJ pot plot continous
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/* Summing up the pairwise interactions. An N^2 algorithm for computing the total energy.  The virial
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/* generates a random position for the test particle within the box 
and calculates its interaction energy with all other particles using the Lennard-Jones potential */
...
int main ( int argc, char * argv[] ) {
  ...
  int N=216,c,a,p;
  double L=0.0;
  double we, w_sum; //Variables for Widom insertion method.
  double beta;
  double rho=0.5, T=1.0, rc2 = 3.5, vir, vir_old, p_sum, pcor, V;
  double E_new, E_old, esum, rr3, ecor, ecut;  //Energy-related variables.
  double ei_new, ei_old, ivir_new, ivir_old; //Energy and virial for individual particle moves.
  double dr=0.2,dx,dy,dz;
  double rxold,ryold,rzold; //Variables to store old positions during trial moves.
  int i,j;
  int nCycles = 10, nSamp, nEq=1000; //Simulation parameters
  int nAcc; //Number of accepted moves.
  int short_out=0; //Flags and counters for various options.
  int shift=0;
  int tailcorr=1;
  int prog=0;
  ...
  pcor = 16.0/3.0*M_PI*rho*rho*(2./3.*rr3*rr3*rr3-rr3); //pressure correction
  ...
  nCycles+=nEq; //Adjusting total cycles to include equilibration cycles
  ...
  for (c=0;c<nCycles;c++) {
    ...
    /*Using the Metropolis criterion to decide whether to accept the move. 
    If accepted, update the total energy and virial. Otherwise, revert the particle to its old position. */
    ...
    if (c>nEq) { //executed only after the equilibration phase
      esum+=E_old;
